const cases = {
  local: {
    ...
    assets: [
      ["1-page ROI sheet", "Manual process cost versus Baseline monthly plan."],
      ["Local lender case study", "Before/after workflow with time saved per loan."],
      ["Spreadsheet replacement checklist", "What breaks when loan volume grows."],
      ["Demo script", "Pipeline, portal, ACH, servicing, payoff."],
      ["Objection card", "Price, migration, onboarding, and data trust."],
    ],
  },
  regional: {
    eyebrow: "Case 02",
    ...
    assets: [
      ["Regional demo script", "Templates, task ownership, draws, reporting."],
      ["Workflow standardization map", "Current fragmented process versus Baseline process."],
      ["Manager dashboard mock", "Pipeline, task aging, draw status, servicing exceptions."],
      ["Borrower journey one-pager", "Application through draw release."],
      ["Implementation plan", "30/60/90 day rollout by team and product line."],
    ],
  },
  national: {
    eyebrow: "Case 03",
    ...
    assets: [
      ["Enterprise security brief", "SOC posture, encryption, permissions, monitoring."],
      ["Capital ops deep dive", "Funds, whole notes, fractional deals, distributions."],
      ["API and migration sheet", "Integration patterns and data transition plan."],
      ["Executive business case", "Risk reduction, reporting speed, operational leverage."],
      ["National lender demo script", "Controls, servicing, capital ops, integrations."],
    ],
  },
};
...
function renderAssets(items) {
  ...
  document.querySelector("#assetCount").textContent = `${items.length} assets`;
}
